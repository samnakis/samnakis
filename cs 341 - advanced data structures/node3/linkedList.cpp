...
// delete node * that is AFTER the node pointed to
void LinkedList::deleteNode(Node * dPtr) {
    // if the list is empty, there's nothing to delete after
    if(isEmpty()) {
        return;
    }
    // if pointing to tail, there's nothing to delete after it
    // also takes care of the scenario where this is the only item in the list
    if(dPtr == tail) {
        return;
    }
    Node * tempPtr = dPtr->next;
    // handles the case where you are deleting the last element of the list
    if(tail == tempPtr) {
        tail == dPtr;
    }
